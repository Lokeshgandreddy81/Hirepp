...
const DEV_BOOTSTRAP_PASSWORD = String(process.env.DEV_BOOTSTRAP_PASSWORD || 'QaUser#12345').trim();
...
router.post('/dev-bootstrap', async (req, res) => {
    ...
    const normalizedRole = requestedRole === 'employer' ? 'employer' : 'worker';
    const loginRole = normalizedRole === 'employer' ? 'recruiter' : 'candidate';
    const activeRole = normalizedRole;
    const email = 'qa.user@hirecircle.local';
    ...
    try {
            ...
            await EmployerProfile.findOneAndUpdate(
                { user: user._id },
                {
                    $setOnInsert: {
                        ...
                        location: 'Hyderabad',
                        industry: 'General',
                        contactPerson: 'QA User',
                        description: 'QA employer profile for local app validation.',
                        country: 'IN',
                    },
                },
                { upsert: true, new: true, setDefaultsOnInsert: true }
        ...
        logger.warn(`Dev bootstrap auth failed: ${error?.message || error}`);
        return res.status(500).json({ message: 'Failed to bootstrap development session' });
    }
});
