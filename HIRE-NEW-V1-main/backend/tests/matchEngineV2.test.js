...
describe('matchEngineV2', () => {
    ...
    it('hard-rejects shift mismatch when both sides are non-flexible', () => {
        const mismatchEvaluation = evaluateRoleAgainstJob({
            job: { ...baseJob, shift: 'Night' },
            worker: { ...baseWorker, preferredShift: 'Day' },
            workerUser: baseUser,
            roleData: baseRole,
        });
        const alignedEvaluation = evaluateRoleAgainstJob({
            job: { ...baseJob, shift: 'Day' },
            worker: { ...baseWorker, preferredShift: 'Day' },
            workerUser: baseUser,
            roleData: baseRole,
        });

        expect(mismatchEvaluation.accepted).toBe(false);
        expect(mismatchEvaluation.rejectReason).toBe(HARD_GATE_REASONS.SHIFT_MISMATCH);
        expect(alignedEvaluation.accepted).toBe(true);
    });
    ...
    it('hard-rejects when skill overlap is below minimum threshold', () => {
        const evaluation = evaluateRoleAgainstJob({
            job: {
                ...baseJob,
                title: 'Driver',
                requirements: ['Driving', 'Fleet Management', '4 years experience'],
                shift: 'Flexible',
            },
            worker: {
                ...baseWorker,
        ...
        expect(evaluation.accepted).toBe(false);
        expect(evaluation.rejectReason).toBe(HARD_GATE_REASONS.SKILL_OVERLAP_BELOW_MINIMUM);
        expect(evaluation.reliabilityMultiplier).toBeUndefined();
    });

    it('bounds reliability multiplier when evaluation is accepted', () => {
        const evaluation = evaluateRoleAgainstJob({
            job: {
                ...baseJob,
                title: 'Driver',
                requirements: ['Driving', 'Route Planning', '3 years experience'],
            },
            worker: {
                ...baseWorker,
                preferredShift: 'Flexible',
            ...
            scoringContext: {
                workerReliabilityScore: 1.2,
                employerStabilityScore: 1.2,
                shiftConsistencyScore: 1.2,
            },
        });

        expect(evaluation.accepted).toBe(true);
        expect(evaluation.reliabilityMultiplier).toBeLessThanOrEqual(1.15);
        expect(evaluation.reliabilityMultiplier).toBeGreaterThanOrEqual(0.85);
    });
