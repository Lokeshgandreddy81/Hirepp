...
const expireOfferAndTransitionApplication = async ({ offer, now }) => {
    if (!offer || !['sent', 'offer_proposed', 'offer_countered'].includes(String(offer.status || '').toLowerCase())) return false;
...
const runOfferExpiryRule = async ({ now, stats }) => {
    ...
    const offersToExpire = await Offer.find({
        status: { $in: ['sent', 'offer_proposed', 'offer_countered'] },
        $or: [
            { expiryDate: { $lte: now } },
...
const evaluateWorkflowSafetyInvariants = async ({ now = new Date() } = {}) => {
    ...
    for (const row of duplicateOfferRows) {
        const offers = await Offer.find({
            ...
            status: { $in: ['sent', 'accepted', 'offer_proposed', 'offer_countered', 'offer_accepted'] },
        }).sort({ createdAt: -1 });

        // Keep the latest active record, cancel older open offers.
        const [, ...duplicates] = offers;
        for (const offer of duplicates) {
            if (['sent', 'offer_proposed', 'offer_countered'].includes(String(offer.status || '').toLowerCase())) {
